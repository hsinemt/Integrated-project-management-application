  ...
  useEffect(() => {
    const fetchProjectsAndGroups = async () => {
      try {
        ...
        // Fetch groups
        const groupsResponse = await axios.get(`${API_URL}/tutor/groups`, {
          headers: {
            Authorization: `${localStorage.getItem("token")}`,
          },
        });
        ...
        if (err.response?.status === 404) {
          setFetchError("API endpoint not found. Please check backend routes.");
        } else if (err.response?.status === 401) {
          setFetchError("Unauthorized. Please log in again.");
        } else {
          setFetchError(err.response?.data?.message || "Failed to load projects or groups");
        }
      }
    };
      ...
      <div className="card shadow-sm">
        ...
        <div className="card-body">
          {dashboardData.groups && dashboardData.groups.length > 0 ? (
            <>
              <div style={{ height: "300px" }} className="mb-4">
                {chartData ? (
                  ...
                  <div className="alert alert-info">No progress data available for chart</div>
                )}
              </div>
            ...
            <div className="alert alert-info">No groups assigned to this tutor</div>
          )}
        </div>
      </div>
    </div>
