...
const TaskBoard = () => {
  ...
  const handleGitBranchUpdate = (taskId: string, gitBranch: string) => {
    setTasks((prevTasks) =>
      prevTasks.map((task) => (task._id === taskId ? { ...task, git: gitBranch } : task))
    );
  };
  ...
  const fetchTasksAndProject = async (token: string) => {
    try {
      ...
      const tasksWithEstimatedTime = await Promise.all(
        response.data.tasks.map(async (task: Task) => {
  ...
  useEffect(() => {
    if (groupId) fetchMessagesWithBearer(groupId);
  }, [groupId]);
  ...
  useEffect(() => {
    ...
    const drake = dragula(containers, {
      moves: (el, source) => source !== completedTasksRef.current,
      accepts: (el, target) => target !== completedTasksRef.current,
    });
  ...
  const handleImageChange = async (taskId: string, event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      try {
        ...
        setTasks((prevTasks) =>
          prevTasks.map((task) => (task._id === taskId ? { ...task, image: response.data.image } : task))
        );
      } catch (error) {
  ...
  const toggleChat = () => {
    ...
    if (!isChatOpen) setIsChatExpanded(true);
  };
  ...
  const sendMessage = (e: React.FormEvent) => {
    ...
    if (newMessage.trim() && groupId && userId) {
      ...
      socket.emit('sendMessage', { groupId, userId, content: newMessage }, (serverMessage: Message) => {
        setMessages((prev) =>
          prev.map((msg) => (msg._id === tempMessageId ? serverMessage : msg))
        );
      });
        ...
        <div className="content">
          <div className="d-flex flex-column align-items-center justify-content-center" style={{ minHeight: '60vh' }}>
            <div className="alert alert-danger text-center" role="alert">
              ...
              <button className="btn btn-primary" onClick={handleRetry} aria-label="Retry loading tasks">
    ...
    <>
          ...
          <div className="card">
            <div className="card-header d-flex align-items-center justify-content-between flex-wrap row-gap-3">
              ...
              <div className="d-flex align-items-center flex-wrap row-gap-3">
                ...
                <button
                  ...
                  style={{ width: '40px', height: '40px', borderRadius: '50%', padding: 0, zIndex: 1000 }}
            ...
            <div className="card-body">
              ...
              <div className="d-flex justify-content-end mt-3">
                <button className="btn btn-primary" onClick={saveTaskStatusChanges}>
                  Save
                </button>
              </div>
            </div>
        ...
        <div className="camera-modal">
          ...
          <div className="webcam-container">
            <Webcam
              ...
              width={500}
              height={375}
              videoConstraints={{ width: 500, height: 375, facingMode: 'user' }}
            />
          </div>
          ...
          {verificationStatus === 'success' && (
            <div className="alert alert-success mt-3">{verificationMessage}</div>
          )}
          {verificationStatus === 'error' && (
            <div className="alert alert-danger mt-3">
              {verificationMessage}
              <button className="btn btn-primary mt-2" onClick={verifyFace}>
                Try Again
              </button>
            </div>
          )}
          <button className="btn btn-secondary mt-3 ml-2" onClick={closeCamera}>
            Close
          </button>
        </div>
      </Modal>
      ...
      >
        <div className="quiz-modal">
          ...
          <button className="btn btn-secondary mt-3 ml-2" onClick={closeQuizModal}>
            Close
          </button>
        </div>
      </Modal>
      ...
      {isChatOpen && (
        ...
        >
          ...
          >
            {isChatExpanded && (
              <h3 style={{ fontSize: '16px', margin: 0, color: '#fff' }}>{groupName || 'Discussion avec le groupe'}</h3>
            )}
            <button
          ...
          {isChatExpanded && (
            <>
              ...
              >
                {chatError ? (
                  ...
                  messages.map((message) => (
                    <div
                      key={message._id}
                      style={{ marginBottom: '10px', textAlign: message.sender._id === userId ? 'right' : 'left' }}
                    >
                      <div
                        style={{
                      ...
                      >
                        ...
                        <p
                          style={{
                            fontSize: '10px',
                            color: message.sender._id === userId ? '#fff' : '#6b7280',
                            marginTop: '4px',
                          }}
                        >
              ...
              <form
                ...
                style={{ padding: '10px', borderTop: '1px solid #e5e7eb', backgroundColor: '#fff' }}
              >
                {showEmojiPicker && (
                ...
                <div style={{ display: 'flex' }}>
                  ...
                  <button
                    ...
                    style={{
                      padding: '8px',
                      background: showEmojiPicker ? '#e5e7eb' : 'none',
                      border: 'none',
                      color: '#64748b',
