...
const TaskColumn = forwardRef<HTMLDivElement, TaskColumnProps>(
  (
    ...
    const columnClasses = `p-3 rounded w-100 me-3 ${
      isCompletedColumn ? 'bg-soft-success' : 'bg-transparent-secondary'
    }`;

    const headerClasses = `p-2 rounded mb-2 ${isCompletedColumn ? 'bg-success text-white' : 'bg-white'}`;
            ...
            <div className="d-flex align-items-center">
              <span
                className={`p-1 d-flex rounded-circle me-2 ${
                  isCompletedColumn ? 'bg-soft-light' : 'bg-soft-purple'
                }`}
              >
                <span
                  className={`rounded-circle d-block p-1 ${
                    isCompletedColumn ? 'bg-light' : 'bg-purple'
                  }`}
                />
              </span>
              <h5 className={`me-2 ${isCompletedColumn ? 'text-white' : ''}`}>{etat}</h5>
              <span
                className={`badge rounded-pill ${
                  isCompletedColumn ? 'bg-light text-dark' : 'bg-light'
                }`}
              >
                {tasks.length}
              </span>
