...
const isUserEmailAvailable = async (req, res) => {
    try {
        const { email } = req.query; // Vérifie dans query string
        console.log("Email reçu :", email); // Vérifie si l'email est bien récupéré
        ...
        if (user) {
            // Si l'utilisateur est trouvé, retourne l'id et 'available' = true
            return res.status(200).json({ available: true, id: user._id });
        } else {
            // Si l'utilisateur n'est pas trouvé, retourne 'available' = false
